public class Percolation {
    ...
    public void open(int i, int j) {
        ...
        if (i >= 0 && i < N) {
            if ((j - 1) >= 0 && (j - 1) < N) {
                if (percolationGrid[i][(j - 1)]) {
                    ...
                    if (quickUnionUF.connected(openedSiteInUFArray, prevOpenedSiteUFArray)) return;
        ...
        if ((i + 1) >= 0 && (i + 1) < N) {
            if (j > 0 && j < N) {
                if (percolationGrid[(i + 1)][j]) {
                    int prevOpenedSiteUFArray = ((i + 1) * this.N) + j;
                    System.out.println(openedSiteInUFArray + " " + prevOpenedSiteUFArray);
                    if (quickUnionUF.connected(openedSiteInUFArray, prevOpenedSiteUFArray)) return;
                    quickUnionUF.union(openedSiteInUFArray, prevOpenedSiteUFArray);
                }
            }
    ...
    public boolean isFull(int i, int j) {
        ...
        if (m >= N || m < 0) throw new java.lang.IndexOutOfBoundsException();
        if (n >= N || n < 0) throw new java.lang.IndexOutOfBoundsException();
        if(quickUnionUF.connected(topVirtualSite, ((m * this.N) + n)) && isOpen(m, n)) return true;
        return false;
    }
